...
* **Root `.env` / `.env.example`**: local Groq configuration / safe template. Real API keys are excluded from the source package.
...
## 4. Review Note Before Day 4

`test_day3_compliance.py` covers 10 refusal categories using controlled mock model responses. This validates the structured refusal contract and schema behavior, but it is **not** an end-to-end retrieval-calibrated refusal precision measurement. Day 4 performs that measurement using real retrieval scores, answerable cases, and unsupported cases.

See [`DAY3_REVIEW.md`](DAY3_REVIEW.md) for the requirement-by-requirement review and the small safety hardening changes made before Day 4.
