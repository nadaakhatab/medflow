...
class TestDay3Compliance(unittest.TestCase):
    ...
    def setUp(self):
        self.sample_chunks = [
            Document(
                page_content="Hypothyroidism is treated by replacing the missing thyroid hormone with synthetic thyroxine pills, which usually have to be taken every day for life.",
                metadata={
    ...
    def test_missing_json_schema_fails_closed(self):
        """Safety hardening: a missing response schema must never silently pass."""
        import generator as generator_module
        original = generator_module.SCHEMA_PATH
        try:
            generator_module.SCHEMA_PATH = os.path.join(os.path.dirname(original), "__missing_schema__.json")
            with self.assertRaises(FileNotFoundError):
                generator_module.validate_with_json_schema({
                    "recommendation": "refuse", "evidence": "", "citations": [], "confidence": "insufficient"
                })
        finally:
            generator_module.SCHEMA_PATH = original
    ...
    def test_supported_question_generates_grounded_answer(self):
        ...
        mock_response = json.dumps({
            "recommendation": "Hypothyroidism is treated by replacing the missing thyroid hormone with synthetic thyroxine pills.",
            "evidence": "Hypothyroidism is treated by replacing the missing thyroid hormone with synthetic thyroxine pills. The recommended replacement dose is typically 1.6 mcg/kg/day.",
            "citations": [
                {
    ...
    def test_human_prompt_does_not_duplicate_system_rules(self):
        """Grounding rules are supplied once as the system message, not duplicated as user text."""
        prompt = build_prompt("Test question", self.sample_chunks)
        self.assertNotIn("citation-bound clinical evidence assistant", prompt.lower())
