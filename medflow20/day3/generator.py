...
def validate_with_json_schema(data: Dict[str, Any]) -> bool:
    """Validate the response against the clinical JSON schema.

    Safety policy: schema validation is fail-closed. A missing schema is a
    configuration error, not permission to bypass response validation.
    """
    if not os.path.exists(SCHEMA_PATH):
        raise FileNotFoundError(f"Clinical response schema not found: {SCHEMA_PATH}")
    try:
        import jsonschema
...
def build_prompt(question: str, retrieved_chunks: List[Union[Document, Dict[str, Any]]]) -> str:
    ...
    # The system grounding rules are supplied once in the system message.
    # Keeping the human message to context + question avoids duplicated instructions.
    return f"""Context:
{context_str}
...
Respond with the JSON object required by the system instructions, nothing else."""
